(function($) {
	...
	function build_menu(settings, obj) {
		var items = $("tbody > tr", obj);
		items.mouseleave(function(e) {
			var to_element = e.relatedTarget;
			if (to_element != null && to_element.localName != "table" && (e.relatedTarget.id != "tablemenu_context_menu" && e.relatedTarget.parentNode.id != "tablemenu_context_menu")) {
				$('#tablemenu_context_menu').hide();
			}
		});
		items.bind(settings.trigger, function(e) {			
			...
			var dk_options = eval('([' + $(this).attr('dk_options') + '])')[0];
			if (settings.width != null) {
				control_width = settings.width;
			} else {
				control_width = Object.keys(dk_options).length * settings.button_width;
			}
			
			if (typeof menu_items['edit'] !== "undefined" && settings.dblclick_action=="edit") {
				if (!settings.disable_dblclick && dk_data.id != null && menu_items['edit']['url'] != null) {
					$(this).bind('dblclick', function() {
						target_url = menu_items['edit']['url'] + dk_data.id;
						if (settings.token != null) target_url += "/" + settings.token;
						window.location.href = target_url;
					});
				}
				
			} else if (settings.dblclick_action!="edit") {
				$(this).bind('dblclick', function() {
					target_url = settings.dblclick_action + "/" + dk_data.id;
					if (settings.token != null) target_url += "/" + settings.token;
					window.location.href = target_url;
				});
				...
				if (dk_options[key] != null) {
					...
					if (val.confirm != null) {
						...
						$.each(placeholders, function(placeholder_key, placeholder_val) {
							replacement = dk_data[placeholder_val];
						...
						$(button).bind('touchstart click', function() {
							if (val.js != null) {
								...
								var js_code = val.js;
								var placeholders = get_placeholders(js_code);

								$.each(placeholders, function(placeholder_key, placeholder_val) {
									replacement = dk_data[placeholder_val];
									js_code = js_code.replace("$(" + placeholder_val + ")", replacement);
								});
			...
			var bottomHeight = $(this).outerHeight() - 1 + "px";
			var lineHeight = $(this).height() - 2 + "px";
			...
			if (settings.disable_select) {
				$(this).parent().bind('selectstart', function(event) {
